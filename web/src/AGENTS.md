...
> **Correction — this codebase does not use Zustand for state management,**
> despite what "State Management Guidelines" and other Zustand-flavored
> examples further down in this file describe (`zustand` is a listed
> dependency in `package.json` but there is no `create<...>()`/`useFlowStore`
> anywhere in `web/src/`). The real state architecture is: `useFlows()`
> (`hooks/useFlows.ts`) is a plain `useState`-based hook that calls the REST
> client (`utils/api.ts`) and sends/receives WebSocket messages via
> `useWebSocket()`; `FlowProvider.tsx` is a thin React Context wrapper around
> `useFlows()`, not a Zustand store. Treat any `useFlowStore`/`zustand`
> reference below as an unimplemented design sketch, not current code.
