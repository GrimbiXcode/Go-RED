...
> **Correction:** the examples below call `useWebSocket(mockUrl)` and use
> `result.current.registerHandler(...)`/`.socket`/`.lastError` at the top
> level. The real `useWebSocket()` takes no arguments and returns
> `{ state: { connected, connecting, error, lastMessage, lastError },
> sendMessage, sendRawMessage, subscribe, unsubscribe, reconnect, close }` —
> use `subscribe(type, callback)` in place of `registerHandler`. See
> `hooks/AGENTS.md`'s correction note and `hooks/useWebSocket.ts` directly
> before writing a new test from these examples.
