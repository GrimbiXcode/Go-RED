	...
	err := reg.RegisterFactory("tls-config", func() registry.NodeExecutor {
		...
		ConfigSchema: registry.Schema{
			Properties: map[string]registry.Property{
				"certType": {
					Type:        "string",
					Description: "files (paths on disk), pem (literal PEM text), or env (env var names holding PEM text)",
					Default:     "files",
					Label:       "Source",
					Order:       1,
					Widget:      "select",
					Options:     []registry.Option{{Value: "files", Label: "File paths"}, {Value: "pem", Label: "PEM text"}, {Value: "env", Label: "Environment variables"}},
				},
				"cert": {
					Type:        "string",
					Description: "Client certificate: path, PEM text, or env var name depending on certType",
					Default:     "",
					Label:       "Certificate",
					Order:       2,
					Widget:      "textarea",
				...
				"key": {
					Type:        "string",
					Description: "Client private key: path, PEM text, or env var name depending on certType",
					Default:     "",
					Label:       "Private key",
					Order:       3,
					Widget:      "textarea",
				...
				"ca": {
					Type:        "string",
					Description: "CA certificate: path, PEM text, or env var name depending on certType",
					Default:     "",
				...
				"servername": {
					Type:        "string",
					Description: "Override the TLS ServerName (SNI) sent to the peer",
					Default:     "",
					Label:       "Server name",
					Placeholder: "as in the certificate",
					Order:       5,
					Widget:      "text",
				},
				"verifyServerCert": {
					Type:        "boolean",
					Description: "Verify the peer's certificate (disable only for testing against a self-signed peer)",
					Default:     true,
