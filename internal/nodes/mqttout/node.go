	...
	err := reg.RegisterFactory("mqtt out", func() registry.NodeExecutor {
		...
		ConfigSchema: registry.Schema{
			Properties: map[string]registry.Property{
				...
				"topic": {
					Type:        "string",
					Description: "Topic to publish to; empty uses msg.topic",
					Default:     "",
					Label:       "Topic",
				...
				"retain": {
					Type:        "boolean",
					Description: "Set the MQTT retain flag",
					Default:     false,
					Label:       "Retain",
					Order:       4,
					Widget:      "boolean",
		...
		Help: "**Publishes `msg.payload`** to an MQTT topic on the selected broker. Leave the topic empty to use `msg.topic`.",
