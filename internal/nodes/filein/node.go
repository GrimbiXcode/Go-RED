	...
	err := reg.RegisterFactory("file in", func() registry.NodeExecutor {
		...
		ConfigSchema: registry.Schema{
			Properties: map[string]registry.Property{
				"filename": {
					Type:        "object",
					Description: `Path to read, e.g. {"type":"str","value":"/tmp/in.txt"}`,
				...
				"format": {
					Type:        "string",
					Description: "\"\" (raw bytes), utf8 (string), or lines (one message per line)",
					Default:     "",
					Label:       "Output",
					Order:       2,
					Widget:      "select",
					Options:     []registry.Option{{Value: "", Label: "Buffer (base64)"}, {Value: "utf8", Label: "Text (UTF-8)"}, {Value: "lines", Label: "One message per line"}},
				},
				"allProps": {
					Type:        "boolean",
					Description: "lines mode: clone the whole input message onto each line instead of only topic/filename",
					Default:     false,
					Label:       "Copy the whole message onto each line",
					Order:       3,
					Widget:      "boolean",
					VisibleWhen: &registry.Condition{Property: "format", Values: []string{"lines"}},
				},
			},
