	...
	err := reg.RegisterFactory("tcp in", func() registry.NodeExecutor {
		...
		ConfigSchema: registry.Schema{
			Properties: map[string]registry.Property{
				"server": {
					Type:        "boolean",
					Description: "true: listen for connections; false: connect out to host:port",
					Default:     true,
					Label:       "Listen (server mode)",
					Order:       1,
					Widget:      "boolean",
				},
				"host": {
					Type:        "string",
					Description: "Remote host (client mode only)",
					Default:     "",
					Label:       "Host",
					Order:       2,
					Widget:      "text",
					VisibleWhen: &registry.Condition{Property: "server", Values: []string{"false"}},
				},
				"port": {
					Type:        "number",
				...
				"splitLines": {
					Type:        "boolean",
					Description: "Split incoming data into one message per line instead of per raw read",
					Default:     false,
					Label:       "One message per line",
					Order:       5,
					Widget:      "boolean",
					VisibleWhen: &registry.Condition{Property: "datatype", Values: []string{"utf8"}},
				},
			},
