	...
	err := reg.RegisterFactory("delay", func() registry.NodeExecutor {
		...
		ConfigSchema: registry.Schema{
			Properties: map[string]registry.Property{
				"mode": {
					Type:        "string",
					Description: "delay (fixed hold per message) or rate (steady release rate)",
					Default:     "delay",
					Label:       "Mode",
					Order:       1,
					Widget:      "select",
					Options:     []registry.Option{{Value: "delay", Label: "Delay each message"}, {Value: "rate", Label: "Rate limit"}},
				},
				"delayMs": {
					Type:        "number",
					...
					Widget:      "duration",
					Unit:        "ms",
					VisibleWhen: &registry.Condition{Property: "mode", Values: []string{"delay"}},
				},
				"rateLimit": {
					Type:        "number",
					...
					Order:       3,
					Widget:      "number",
					VisibleWhen: &registry.Condition{Property: "mode", Values: []string{"rate"}},
				},
				"rateIntervalMs": {
					Type:        "number",
				...
				"dropIntermediate": {
					Type:        "boolean",
					Description: "rate mode: keep only the most recently queued message instead of queuing every one",
					Default:     false,
					Label:       "Drop intermediate messages",
					Order:       5,
					Widget:      "boolean",
					VisibleWhen: &registry.Condition{Property: "mode", Values: []string{"rate"}},
				},
